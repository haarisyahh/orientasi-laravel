...
<head>
  ...
  <title>Log Masuk Penyelia</title>
...
<body class="min-h-screen bg-gray-100 p-4 flex items-center justify-center">
  ...
  <div class="w-full max-w-md rounded-2xl bg-white/95 p-5 shadow-2xl">
    <h1 class="text-xl font-bold text-center">LOG MASUK PENYELIA</h1>
    ...
    <form method="POST" action="{{ route('login.penyelia.submit') }}" class="space-y-3">@csrf
      ...
      <div class="rounded bg-blue-100 px-3 py-2 text-xs text-blue-900 border-l-4 border-blue-500">
        <strong>Nota:</strong> Jika terlupa kata laluan, sila berjumpa Unit Pengurusan Maklumat (UPM) atau hubungi Ext No: 3001 untuk reset kata laluan. Selepas reset oleh Admin, gunakan kata laluan sementara <strong>User000</strong> dan anda wajib tukar kata laluan selepas log masuk.
      </div>
      <button class="w-full rounded bg-emerald-600 py-2 text-sm font-semibold text-white hover:bg-emerald-700">Log Masuk</button>
    </form>
    <p class="mt-3 text-center text-xs text-gray-600">Belum mendaftar? <a class="font-semibold text-blue-600" href="{{ route('register.penyelia') }}">Daftar Penyelia</a></p>
  </div>
</body>
</html>
