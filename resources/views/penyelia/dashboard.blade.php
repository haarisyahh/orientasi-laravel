...
<body class="min-h-screen bg-gray-100 flex flex-col">
  ...
  @if($mustChangePassword)
    <div class="bg-amber-400 text-amber-900 px-4 py-3 text-sm font-semibold text-center flex items-center justify-center gap-2">
      ...
      Anda masih menggunakan kata laluan default. Sila tukar kata laluan sekarang untuk meneruskan penggunaan sistem.
      <button onclick="document.getElementById('changePwModal').classList.remove('hidden')" class="ml-2 underline font-bold hover:text-amber-700">Tukar Sekarang</button>
    </div>
  @endif

  @if(session('status'))
    <div class="bg-emerald-100 text-emerald-800 px-4 py-3 text-sm font-semibold text-center">{{ session('status') }}</div>
  @endif
              ...
              @forelse($staffRows as $i => $row)
                @php
                  ...
                  $completionDate = $row->completion_date
                    ? \Carbon\Carbon::parse($row->completion_date)->format('d/m/Y H:i')
                    : '-';
                @endphp
  ...
  <div id="changePwModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 {{ $mustChangePassword ? '' : 'hidden' }}">
    <div class="w-full max-w-md rounded-xl bg-white shadow-2xl overflow-hidden">
      <div class="bg-amber-400 px-5 py-4">
        <h2 class="text-base font-extrabold text-amber-900 flex items-center gap-2">
        ...
        <p class="text-xs text-amber-800 mt-1">Anda mesti menukar kata laluan default <strong>User000</strong> sebelum boleh menggunakan sistem.</p>
      </div>

      <form method="POST" action="{{ route('penyelia.change-password') }}" class="px-5 py-4 space-y-3">
        @csrf

        @if($errors->any())
          <div class="rounded border border-red-300 bg-red-50 px-3 py-2 text-xs text-red-700">
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-700 mb-1">Kata Laluan Baru</label>
          <input type="password" name="password" class="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" placeholder="Minimum 8 aksara" required autofocus>
        </div>
        <div>
          <label class="block text-xs font-semibold text-gray-700 mb-1">Sahkan Kata Laluan Baru</label>
          <input type="password" name="password_confirmation" class="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" placeholder="Ulang kata laluan" required>
        </div>

        <div class="pt-1 flex gap-2">
          <button type="submit" class="flex-1 rounded bg-blue-700 py-2 text-sm font-bold text-white hover:bg-blue-800">Tukar Kata Laluan</button>
          @if(!$mustChangePassword)
            <button type="button" onclick="document.getElementById('changePwModal').classList.add('hidden')" class="rounded border border-gray-300 px-4 py-2 text-xs font-semibold text-gray-600 hover:bg-gray-50">Batal</button>
          @endif
        </div>
      </form>
    </div>
