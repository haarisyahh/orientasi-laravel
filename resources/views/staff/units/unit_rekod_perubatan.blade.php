...
@elseif($isRekodVideoSlide)
  <div class="video-title">{{ $rekodVideoTitle }}</div>
  <div class="video-wrapper" id="slideImage">
    <video class="video-frame" controls playsinline controlsList="noplaybackrate noremoteplayback">
      <source src="{{ asset($rekodVideoFile) }}" type="video/mp4">
      Browser anda tidak menyokong video.
    </video>
    <a class="video-download" href="{{ asset($rekodVideoFile) }}" download="{{ $rekodVideoTitle }}.mp4"></a>
  </div>
