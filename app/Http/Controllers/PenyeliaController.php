...
class PenyeliaController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        $request->session()->put([
            ...
            'role' => $penyelia->role,
            'penyelia_must_change_password' => Hash::check('User000', $penyelia->password),
        ]);
    ...
    public function dashboard(Request $request)
    {
        ...
        $mustChangePassword = (bool) $request->session()->get('penyelia_must_change_password', false);
    ...
    public function changePassword(Request $request)
    {
        ...
        $data = $request->validate([
            'password' => ['required', 'string', 'min:8', 'confirmed', 'not_in:User000'],
        ], [
            'password.required' => 'Kata laluan diperlukan.',
            'password.min' => 'Kata laluan mesti sekurang-kurangnya 8 aksara.',
            'password.confirmed' => 'Pengesahan kata laluan tidak sepadan.',
            'password.not_in' => 'Kata laluan baru tidak boleh sama dengan kata laluan default.',
        ]);

        $penyeliaId = (int) $request->session()->get('penyelia_id');

        DB::table('penyelia')->where('id', $penyeliaId)->update([
            'password' => Hash::make((string) $data['password']),
            'updated_at' => now(),
        ]);

        $request->session()->put('penyelia_must_change_password', false);

        return redirect()->route('penyelia.dashboard')->with('status', 'Kata laluan berjaya ditukar. Terima kasih!');
    }
