...
<body class="bg-gray-100 min-h-screen">
  <div class="max-w-7xl mx-auto p-4">
    <div class="mb-4 flex items-center justify-between gap-2 flex-wrap">
      <h1 class="text-xl font-bold">Pengurusan Penyelia</h1>
        ...
        <tbody>
          @forelse($rows as $row)
            <tr class="border-t">
              ...
              <td class="px-3 py-2">{{ $row->created_at ? \Carbon\Carbon::parse($row->created_at)->format('d/m/Y H:i') : '-' }}</td>
              <td class="px-3 py-2">
  ...
  <div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-md rounded bg-white p-4 shadow-lg">
      <h2 class="mb-3 text-lg font-semibold">Edit Penyelia</h2>
      <form method="POST" action="{{ route('admin.pengurusan-penyelia.edit-user') }}" class="space-y-2">
        @csrf
