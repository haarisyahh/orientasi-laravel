  ...
  <div class="max-w-7xl mx-auto p-4">
    <div class="mb-4 flex items-center justify-between gap-2 flex-wrap">
      ...
      <div class="flex gap-2 flex-wrap">
        <button onclick="window.print()" class="no-print rounded bg-blue-600 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-700">
          🖨️ Cetak
        </button>
        <a href="{{ route('admin.pengurusan-staff.export-csv') }}{{ $search ? '?search=' . urlencode($search) : '' }}" class="no-print rounded bg-green-600 px-3 py-2 text-xs font-semibold text-white hover:bg-green-700">
          📥 Export CSV
        </a>
        <a href="{{ route('admin.dashboard') }}" class="no-print rounded bg-slate-700 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-800">Kembali Dashboard</a>
      </div>
    </div>
    ...
    <form method="GET" class="no-print mb-4 flex gap-2">
      ...
      <table class="min-w-full text-sm">
        <thead class="bg-gray-100">
          <tr>
            ...
            <th class="px-3 py-2 text-left">Tarikh Daftar</th>
            <th class="px-3 py-2 text-left">Status</th>
            <th class="no-print px-3 py-2 text-left">Tindakan</th>
          </tr>
        </thead>
        <tbody>
          @forelse($rows as $row)
            ...
            <tr class="border-t">
              ...
              <td class="px-3 py-2 text-sm">{{ $tarikhDaftar }}</td>
          ...
          @empty
            <tr><td colspan="8" class="px-3 py-4 text-center text-gray-500">Tiada rekod ditemui.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
